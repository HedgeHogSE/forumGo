...
import (
	"database/sql"
	"forum/backend/forum/external"
	models2 "forum/backend/forum/models"
...
func GetTopicWithData(c *gin.Context) {
	type TopicWithData struct {
		...
		Comments    []models2.Comment `json:"comments"`
		AuthorID    int               `json:"author_id"`
	}

	if topicID, err := strconv.Atoi(c.Param("topic_id")); err == nil {

		if topic, err := models2.GetTopicByID(topicID); err == nil {
			...
			comments, err1 := models2.GetCommentsByTopicID(topicID)
			if err1 != nil {
				panic(err1)
			}
			res := TopicWithData{
...
func PostNewTopic(c *gin.Context) {
	...
	models2.AddTopic(&models2.Topic{Title: newTopic.Title,
...
func DeleteTopic(c *gin.Context) {
	if topicID, err := strconv.Atoi(c.Param("topic_id")); err == nil {
		models2.DeleteTopicByID(topicID)
...
func PutTopic(c *gin.Context) {
	...
	updated, err := models2.PutTopic(id, &models2.Topic{
